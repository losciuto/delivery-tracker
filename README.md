...
├── gui/                 # Package interfaccia grafica
│   ├── __init__.py      # Espone MainWindow
│   ├── main_window.py   # Logica principale finestra
│   └── dialogs.py       # Dialoghi (Ordine, Impostazioni)
...
- ✅ **Dati Prodotto**: Nuovi campi per Prezzo e Link Immagine con visualizzazione in tabella.
- ✅ **IA Scraping**: Workflow `/import-temu` per l'estrazione automatizzata di ordini complessi via browser.
- ✅ **Cleanup UI**: Rimozione delle procedure di importazione HTML/URL obsolete per una maggiore stabilità.

## 🔄 Aggiornamenti v2.2.1
- ✅ **Ottimizzazione UI**: Layout migliorato per monitor da 19" e riorganizzazione sidebar.
- ✅ **Azioni Rapide**: Pulsante Aggiorna spostato in posizione prioritaria.
...
- ✅ **Email Sync 2.0**:
    ...
    - Priorità estrazione Order ID rispetto al Tracking per evitare sovrapposizioni.
    - Logica "Smart State": priorità all'oggetto email e riconoscimento conferme d'ordine.
- ✅ **UI & UX**:
    - **Raggruppamento Ordini**: Evidenziazione dinamica degli articoli appartenenti allo stesso ID Ordine.
    - **Indicatore Visivo**: Icona 🔗 per ordini multipli.
...
## 🔄 Aggiornamenti v2.1.1
- ✅ **Fix Impostazioni**: Campo email ora modificabile manualmente.
